...
import { DisconnectReason, EaglerPacketId, MAGIC_BUILTIN_SKIN_BYTES, MAGIC_ENDING_IDENTIFY_S_BYTES } from "./eaglerPacketDef.js";
import { Logger } from "./logger.js";
import { ChatColor, State } from "./types.js";
import { toBuffer, toString as uuidToString } from "uuid-buffer";
import * as mc from "minecraft-protocol";
import { config } from "./config.js";
import sharp from "sharp";
import { createHash } from "crypto";
import { processClientReqPacket } from "./eaglerSkin.js";
...
export function chatToPlainString(chat) {
    let ret = '';
    if (chat.text != null)
        ret += chat.text;
    if (chat.extra != null) {
...
export function disconnect(player, message, code) {
    if (player.state == State.POST_HANDSHAKE) {
        ...
        const messageLen = encodeVarInt(message_m.length), codeEnc = encodeVarInt(code !== null && code !== void 0 ? code : DisconnectReason.CUSTOM);
...
const ICON_SQRT = 64;
export function generateMOTDImage(file) {
    return new Promise((res, rej) => {
        sharp(file)
